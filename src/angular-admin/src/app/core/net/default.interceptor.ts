...
export class DefaultInterceptor implements HttpInterceptor {
  ...
  private handleData(ev: HttpResponseBase, req: HttpRequest<any>, next: HttpHandler): Observable<any> {
    ...
    if (ev instanceof HttpErrorResponse && ev.headers.get('_AbpErrorFormat')) {
      const body = snq(() => ev.error.error, DEFAULT_ERROR_MESSAGES.defaultError.title);
      const confirmation$ = this.showError(undefined, undefined, body);
      if (ev.status === 401) {
      ...
      switch (ev.status) {
        ...
        case 401:
          ...
          if (ev.url!.indexOf('app') <= 0 && ev.url!.indexOf('application-configuration') <= 0) {
            (this.injector.get(DA_SERVICE_TOKEN) as ITokenService).clear();
            this.goTo('/passport/login');
          }
          break;
...
function snq(arg0: () => any, title: string) {
